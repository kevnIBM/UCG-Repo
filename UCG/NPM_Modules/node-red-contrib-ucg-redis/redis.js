module.exports = function(RED) {
    ...
    RED.nodes.registerType("redis-ucg-config", RedisConfig);
    ...
    RED.nodes.registerType("redis-ucg-in", RedisIn);
    ...
    RED.nodes.registerType("redis-ucg-out", RedisOut);
    ...
    function RedisCmd(n) {
        ...
        var client = connect(node.server);
        var config_env = _setEnv(node.server);
    ...
    RED.nodes.registerType("redis-ucg-command", RedisCmd);
    ...
    RED.nodes.registerType("redis-lua-ucg-script", RedisLua);
    ...
    function connect(config, force) {
        ...
        if (force !== undefined || usingConn[idx] === undefined || usingConn[idx] === 0) {
            ...
            conn.auth(config_env.pass,function (err, result) {
              if (err) {
                console.error(err); }
            }); 
