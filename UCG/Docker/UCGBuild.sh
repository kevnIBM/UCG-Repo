...
docker build -f latest/Dockerfile-Data-Volume-Mapped -t dennisnotojr/node-red-docker-node8-nr-17-data-mapped:version-5 .

# Build command for creating the container for volume mapping via the run command and using cluster service
docker build -f latest/Dockerfile-Data-Volume-Mapped-Cluster -t dennisnotojr/node-red-docker-node8-nr-17-data-mapped-cluster:version-4 .
...
docker push dennisnotojr/node-red-docker-node8-nr-17-data-mapped:version-5
docker push dennisnotojr/node-red-docker-node8-nr-17-data-mapped-cluster:version-4
